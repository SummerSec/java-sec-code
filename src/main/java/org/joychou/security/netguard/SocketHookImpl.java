...
public class SocketHookImpl extends SocketImpl implements SocketOptions {
    ...
    protected void connect(InetAddress address, int port) {
        ...
        try {
            if (UrlGuard.isInternalIp(address.getHostAddress())) {
                throw new RuntimeException("Socket url check failed. InetAddress:" + address.toString());
            }
    ...
    protected void connect(SocketAddress address, int timeout) {
        ...
        try {
            if (UrlGuard.isInternalIp(ip)) {
                throw new UrlGuardException(String.format("[-] url check failed. Hostname: %s IP: %s", host, ip));
            }
